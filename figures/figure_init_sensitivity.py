...
for j, (model_path, res_cont_name, LT) in enumerate(
    ...
    if LT == 1.0:
        ...
        ax[0].xaxis.set_major_locator(MultipleLocator(2.5))
        ax[1].xaxis.set_major_locator(MultipleLocator(2.5))
    else:
...
if save_fig:
    fig.savefig(
        f"paper/graphics/figure_init_cont_sensitivity.png",
        bbox_inches="tight",
        dpi=300,
    )
    fig.savefig(
        f"paper/graphics/figure_init_cont_sensitivity.pdf",
        bbox_inches="tight",
    )
plt.show()
