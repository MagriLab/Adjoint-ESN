...
from adjoint_esn.utils.dynamical_systems import Lorenz96
...
def main(_):
    ...
    if not FLAGS.experiment_path:
        ...
        FLAGS.experiment_path = (
            Path.cwd() / "local_results" / "lorenz96" / f"run_{dt_string}"
        )
    ...
    eParam = Lorenz96.get_eParamVar()
    param_mesh_input = [None] * 1
    param_mesh_input[eParam.p] = config.simulation.p_list
    ...
    for p in train_param_list[:, eParam.p]:
        print(f"p = {p}")

    val_param_list = param_list[new_list_val_idx]
    print("Validation regimes:")
    for p in val_param_list[:, eParam.p]:
        print(f"p = {p}")
