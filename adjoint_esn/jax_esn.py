...
class JAXESN:
    def __init__(
        ...
        parameter_normalization=jnp.array([[0.0], [1.0]]),
        input_scaling=1.0,
        spectral_radius=1.0,
        leak_factor=1.0,
        input_bias=1.0,
        ...
        et = time.time()
        print("Input weight generation", et - st)
        ...
        et = time.time()
        print("Reservoir weight generation", et - st)
    ...
    #    W_in = jnp.zeros(self.W_in_shape)
    #    # set the seeds
    #    key = random.PRNGKey(self.W_in_seeds)
    #    key1, key2 = random.split(key)
    #
    #    # make W_in
    #    for j in range(self.N_reservoir):
    #        key1, *keys1 = random.split(key1,3)
    #        rnd_idx = random.randint(keys1[0],shape = (1,), minval = 0, maxval = self.N_dim+1)
    #        # only one element different from zero
    #        # sample from the uniform distribution
    #        W_in = W_in.at[j,rnd_idx].set(random.uniform(keys1[1], shape=(1,),minval=-1,maxval=1))
    #    # input associated with system's bifurcation parameters are
    #    # fully connected to the reservoir states
    #    if self.N_param_dim > 0:
    #        W_in = W_in.at[:,-self.N_param_dim :].set(random.uniform(key2, shape=(self.N_reservoir, self.N_param_dim),minval=-1,maxval=1))
    #    return W_in
    #
    ...
    #    W = jnp.zeros(self.W_shape)
    #
    #    # set the seeds
    #    key = random.PRNGKey(self.W_seeds)
    #    key1, key2 = random.split(key)
    #
    #    # generate a matrix sampled from the uniform distribution (0,1)
    #    W_connection = random.uniform(key1, shape=self.W_shape, minval=0, maxval=1)
    #
    #    # generate the weights from the uniform distribution (-1,1)
    #    W_weights = random.uniform(key2, shape=self.W_shape, minval=-1, maxval=1)
    #
    #    # replace the connections with the weights
    #    W = jnp.where(W_connection < (1 - self.sparseness), W_weights, W)
    ...
    #    rho_pre = np.abs(sparse_eigs(np.array(W), k=1, which="LM", return_eigenvectors=False))[0]
    ...
    def generate_input_weights(self):
        ...
        return BCOO.from_scipy_sparse(W_in)
        # return W_in

    # TODO: move to different file with different methods
    def generate_reservoir_weights(self):
        """Create the reservoir weights matrix according to Erdos-Renyi network
        Args:
            seeds: a list of seeds for the random generators;
        ...
        return BCOO.from_scipy_sparse(W)
        # return W

    # @ TODO: create base_step function for just the stepping, do the other stuff in other functions
    def step(self, x_prev, u, p=None):
        ...
        u_augmented = jnp.hstack((u_norm, self.b_in))

        # augment the input with the parameters
        if self.N_param_dim > 0:
            u_augmented = jnp.hstack(
                (u_augmented, (p - self.norm_p[0]) / self.norm_p[1])
            )

        # update the reservoir
        x_tilde = jnp.tanh(self.W_in @ u_augmented + self.W @ x_prev)
    ...
    def open_loop(self, x0, U, P=None):
        ...
        step_jit = jit(self.step)
        # update the reservoir
        # step in time
        # @TODO: replacing is slow, adding is fast
        # python loops are slow, replace with jax.lax.fori_loop
        # if statement. if can't get rid of it, jax.lax.cond
        for n in range(1, N_t + 1):
            # update the reservoir
            if self.N_param_dim > 0:
                X.append(step_jit(X[n - 1], U[n - 1], P[n - 1]))
            else:
                X.append(step_jit(X[n - 1], U[n - 1]))
        X = jnp.array(X)
        return X
    ...
    def dfdu_const(self):
        if not hasattr(self, "_dfdu_const"):
            ...
            self._dfdu_const = jnp.array(self._dfdu_const.toarray())
    ...
    def dudr(self):
        if not hasattr(self, "_dudr"):
            ...
            self._dudr = jnp.array(self._dudr)
        return self._dudr
    ...
    def dfdu_dudr_const(self):
        if not hasattr(self, "_dfdu_dudr_const"):
            ...
            self._dfdu_dudr_const = jnp.array(self._dfdu_dudr_const)
    ...
    def dfdr_r_const(self):
        if not hasattr(self, "_dfdr_r_const"):
            ...
            self._dfdu_dudr_const = jnp.array(self._dfdr_r_const())
    ...
    def jac(self, x):
        ...
        dfdr_u = jnp.multiply(self.dfdu_dudr_const, dtanh)
    ...
    def drdp_const(self):
        if not hasattr(self, "_drdp_const"):
            ...
            self._drdp_const = jnp.array(self._drdp_const.toarray())
    ...
    def drdp(self, x):
        ...
        dtanh = 1.0 - x**2
        dtanh = dtanh[:, None]
        drdp = jnp.multiply(self.drdp_const, dtanh)
        return drdp
